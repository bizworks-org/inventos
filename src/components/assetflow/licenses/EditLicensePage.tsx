...
const complianceStatuses: License["compliance"][] = [
  "Compliant",
  "Warning",
  "Non-Compliant",
];
...
export function EditLicensePage({
    ...
    try {
      ...
      if (s) {
        ...
        if (Array.isArray(parsed.licenseFields))
          setFieldDefs(parsed.licenseFields as AssetFieldDef[]);
      }
    } catch {}
  }, []);
  ...
  useEffect(() => {
    if (!license) return;
    if (
      (license as any).customFields &&
      typeof (license as any).customFields === "object"
    ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    logLicenseCreated(updated.id, updated.name, "admin@company.com", {
      seats: updated.seats,
      cost: updated.cost,
    });
  ...
  return (
    <AssetFlowLayout
      breadcrumbs={[
        { label: "Home", href: "#" },
        { label: "Licenses", href: "#" },
        { label: license ? license.name : "Edit License" },
      ]}
      currentPage="licenses"
      onSearch={onSearch}
    >
              ...
              <div className="space-y-3">
                <Button
                  ...
                  className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-semibold transition-all duration-200 ${
                    saving
                      ? "bg-white/70 text-[#6366f1]/60 cursor-not-allowed"
