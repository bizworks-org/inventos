...
export default function SearchResultsPage({ query, onNavigate, onSearch }: SearchResultsPageProps) {
  ...
  useEffect(() => {
    if (!q) {
      setAssets([]); setVendors([]); setLicenses([]);
      setAssetsTotal(0); setVendorsTotal(0); setLicensesTotal(0);
      setLoading(false);
      return;
    }
    let cancelled = false;
    ...
    >
      <div className="mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[#1a1d2e]">Search results</h1>
            <p className="text-sm text-[#64748b]">Results for <span className="font-medium">{query}</span></p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-sm text-[#64748b]">{assetsTotal} matches</span>
            </div>
            {assets.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-3">
                {assets.map(a => (
                  <motion.div key={a.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="p-4 bg-white rounded-lg border border-[rgba(0,0,0,0.06)] flex items-center justify-between">
                    <div>
                      <div className="font-semibold text-[#1a1d2e]">{a.name}</div>
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-sm text-[#64748b]">{vendorsTotal} matches</span>
            </div>
            {vendors.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-3">
                {vendors.map(v => (
                  <motion.div key={v.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="p-4 bg-white rounded-lg border border-[rgba(0,0,0,0.06)] flex items-center justify-between">
                    <div>
                      <div className="font-semibold text-[#1a1d2e]">{v.name}</div>
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-sm text-[#64748b]">{licensesTotal} matches</span>
            </div>
            {licenses.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-3">
                {licenses.map(l => (
                  <motion.div key={l.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="p-4 bg-white rounded-lg border border-[rgba(0,0,0,0.06)] flex items-center justify-between">
                    <div>
                      <div className="font-semibold text-[#1a1d2e]">{l.name}</div>
