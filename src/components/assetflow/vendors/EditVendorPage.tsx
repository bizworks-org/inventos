...
export function EditVendorPage({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (fieldDefs.length > 0) {
      (updated as any).customFields = Object.fromEntries(
        fieldDefs.map((def) => [def.key, customFieldValues[def.key] ?? ""])
      );
    }

    // include extended fields if present in formData
    (updated as any).legalName =
      (formData as any).legalName ?? (vendor as any).legalName ?? undefined;
    (updated as any).tradingName =
      (formData as any).tradingName ?? (vendor as any).tradingName ?? undefined;
    (updated as any).registrationNumber =
      (formData as any).registrationNumber ??
      (vendor as any).registrationNumber ??
      undefined;
    (updated as any).incorporationDate =
      (formData as any).incorporationDate ??
      (vendor as any).incorporationDate ??
      undefined;
    (updated as any).incorporationCountry =
      (formData as any).incorporationCountry ??
      (vendor as any).incorporationCountry ??
      undefined;
    (updated as any).registeredOfficeAddress =
      (formData as any).registeredOfficeAddress ??
      (vendor as any).registeredOfficeAddress ??
      undefined;
    (updated as any).corporateOfficeAddress =
      (formData as any).corporateOfficeAddress ??
      (vendor as any).corporateOfficeAddress ??
      undefined;
    (updated as any).natureOfBusiness =
      (formData as any).natureOfBusiness ??
      (vendor as any).natureOfBusiness ??
      undefined;
    (updated as any).businessCategory =
      (formData as any).businessCategory ??
      (vendor as any).businessCategory ??
      undefined;
    (updated as any).serviceCoverageArea =
      (formData as any).serviceCoverageArea ??
      (vendor as any).serviceCoverageArea ??
      undefined;

    // include contacts (up to 5)
    const contactsFromForm = (formData as any).contacts;
    if (Array.isArray(contactsFromForm)) {
      (updated as any).contacts = contactsFromForm
        .slice(0, 5)
        .map((c: any) => ({ ...c }));
    ...
    (updated as any).accountNumber =
      ...
      undefined;
    (updated as any).ifscSwiftCode =
      (formData as any).ifscSwiftCode ??
      (vendor as any).ifscSwiftCode ??
      undefined;
    (updated as any).paymentTerms =
      (formData as any).paymentTerms ??
              ...
              >
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div
                      key={docDef.key}
                      className="p-3 border rounded-lg bg-[#fbfbff]"
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="text-sm font-medium">
                          {docDef.label}
                      ...
                      <div className="mt-3">
                        ...
                        <div className="space-y-2">
                          ...
                          {formDocs.filter((d) => d.type === docDef.key)
                            .length === 0 && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange("panTaxId", e.target.value)
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange("ifscSwiftCode", e.target.value)
                      }
                  ...
                  <div className="md:col-span-2">
                    ...
                    {formData.gstCertificateName && (
                      <p className="text-sm text-[#64748b] mt-2">
                        Current file:{" "}
                        <strong>{formData.gstCertificateName}</strong>
                      </p>
                    )}
                  </div>
                </div>
