    ...
    try {
      ...
      setTimeout(() => {
        const els = Array.from(
          document.querySelectorAll('[data-test^="asset-delete-"]')
        );
        if (els.length) {
          console.info("DEBUG: Found delete buttons:", els.length);
          for (const el of els) {
            console.info(
              "DEBUG: delete-button classList ->",
...
function IconBox({ asset, iconBox, categoryOfAsset }: Readonly<{ asset: Asset; iconBox: string; categoryOfAsset: (asset: Asset) => string }>) {
  ...
  const name =
    (typeof (asset as any).type_name === "string" && (asset as any).type_name) ||
    ((asset as any).type_id ?? (asset as any).type ?? "");
  return (
    <div className={`${iconBox} rounded-lg flex items-center justify-center font-semibold ${classes}`}>
...
function AssetDetails({
  ...
  const specsRaw =
    (asset as any).specifications ?? (asset as any).specs ?? (asset as any).specs_json ?? null;
  ...
  const rows: Array<{ label: string; content: React.ReactNode }> = [
    ...
    {
      label: "Type",
      content: v(resolveTypeName(asset) || (asset as any).type || (asset as any).type_id || (asset as any).typeId),
    },
    { label: "Serial Number", content: v((asset as any).serialNumber || (asset as any).serial_number) },
    { label: "Assigned To", content: v((asset as any).assignedTo || (asset as any).assigned_to) },
    { label: "Assigned Email", content: v((asset as any).assignedEmail || (asset as any).assigned_email) },
    { label: "Consent Status", content: v((asset as any).consentStatus || (asset as any).consent_status) },
    { label: "Department", content: v((asset as any).department || (asset as any).dept) },
    { label: "Vendor", content: v((asset as any).vendor) },
    { label: "Status", content: v(asset.status || (asset as any).status) },
    { label: "Purchase Date", content: tryDate((asset as any).purchaseDate || (asset as any).purchase_date) },
    {
      label: "End Of Support",
      content: tryDate(
        (asset as any).eosDate || (asset as any).end_of_support_date || (asset as any).endOfSupportDate
      ),
    },
    {
      label: "End Of Life",
      content: tryDate((asset as any).eolDate || (asset as any).end_of_life_date || (asset as any).endOfLifeDate),
    },
    { label: "Warranty Expiry", content: tryDate((asset as any).warrantyExpiry || (asset as any).warranty_expiry) },
    {
      label: "Cost",
      content: v(formatCurrency(Number((asset as any).cost ?? (asset as any).price ?? 0))),
    },
  ...
  if (specs && Object.keys(specs).length) {
    const specNodes = Object.entries(specs).map(([k, val]) => (
      <div key={k} className="text-sm text-gray-900 dark:text-gray-100">
        <span className="text-xs text-gray-600 dark:text-gray-400 mr-2">{k}:</span> {String(val)}
      </div>
    ));
    rows.push({ label: "Specifications", content: <div className="flex flex-col gap-1">{specNodes}</div> });
  } else {
    rows.push({ label: "Specifications", content: v((asset as any).specifications || "—") });
  }

  rows.push({ label: "CIA - Confidentiality", content: v(cia_c) });
  rows.push({ label: "CIA - Integrity", content: v(cia_i) });
  rows.push({ label: "CIA - Availability", content: v(cia_a) });
  rows.push({ label: "CIA - Total", content: String(ciaTotal) });
  rows.push({ label: "CIA - Score (avg)", content: ciaScore ? ciaScore.toFixed(2) : "—" });

  rows.push({ label: "Created At", content: tryDate((asset as any).created_at || (asset as any).createdAt) });
  rows.push({ label: "Updated At", content: tryDate((asset as any).updated_at || (asset as any).updatedAt) });
  ...
  return (
    <motion.tr key={`${asset.id}-details`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.15 }}>
      <td
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {rows.map((r) => (
            <div key={r.label}>
              <div className="text-xs text-gray-600 dark:text-gray-400">{r.label}</div>
              <div className="font-medium text-gray-900 dark:text-gray-100">{r.content}</div>
            </div>
          ))}
        </div>
      </td>
    ...
    <>
      <motion.tr
        ...
        className={`border-b border-gray-100 dark:border-gray-800 transition-all duration-200 ease-in-out
          ${hoveredRow === asset.id ? "bg-gradient-to-r from-gray-50 to-transparent dark:from-gray-900 dark:to-transparent" : ""}
          ${expandedId === asset.id ? "bg-[#eef2ff] shadow-md border-l-4 border-[#6366f1] animate-pulse" : ""}`}
      >
        <td className={`${cellPad}`}>
          <div className="flex items-center gap-3">
            <IconBox asset={asset} iconBox={iconBox} categoryOfAsset={categoryOfAsset} />
            <div>
              <p className={`font-semibold text-gray-900 dark:text-gray-100 ${nameText}`}>{asset.name}</p>
              <p className={`${subText} text-gray-500 dark:text-gray-400`}>{resolveTypeName(asset)}</p>
            </div>
          </div>
        </td>

        <td className={`${cellPad}`}>
          <p className={`text-sm text-gray-600 dark:text-gray-400 font-mono ${density === "ultra-compact" ? "text-[12px]" : ""}`}>
            {asset.serialNumber}
          </p>
        </td>

        <td className={`${cellPad}`}>
          <p className={`text-sm text-gray-900 dark:text-gray-100 font-medium ${density === "ultra-compact" ? "text-[12px]" : ""}`}>
            {asset.assignedTo}
          </p>
        </td>

        <td className={`${cellPad}`}>
          <p className={`text-sm text-gray-600 dark:text-gray-400 ${density === "ultra-compact" ? "text-[12px]" : ""}`}>
            {asset.department}
          </p>
        </td>
        ...
        {consentRequired && (
          <td className={`${cellPad}`}>
            <div className="flex items-center gap-2">
              {consentBadge(asset)}
              {asset.assignedEmail && <span className={`text-xs text-[#6b7280] ${density === "ultra-compact" ? "hidden" : ""}`}>{asset.assignedEmail}</span>}
            </div>
          </td>
        )}
        ...
        <td className={`${cellPad}`}>
          <div>
            <p className={`text-sm font-medium ${isDateExpiring((asset as any).eosDate || "") ? "text-[#f59e0b]" : "text-gray-600 dark:text-gray-400"} ${density === "ultra-compact" ? "text-[12px]" : ""}`}>
              {(asset as any).eosDate ? formatDate((asset as any).eosDate) : "—"}
            </p>
            {isDateExpiring((asset as any).eosDate || "") && <p className={`${subText} text-[#f59e0b]`}>Expiring soon</p>}
          </div>
        </td>

        <td className={`${cellPad}`}>
          <div>
            <p className={`text-sm font-medium ${isDateExpiring((asset as any).eolDate || "") ? "text-[#f59e0b]" : "text-gray-600 dark:text-gray-400"} ${density === "ultra-compact" ? "text-[12px]" : ""}`}>
              {(asset as any).eolDate ? formatDate((asset as any).eolDate) : "—"}
            </p>
            {isDateExpiring((asset as any).eolDate || "") && <p className={`${subText} text-[#f59e0b]`}>Expiring soon</p>}
          </div>
        </td>

        <td className={`${cellPad}`}>
          <p className={`text-sm font-semibold text-gray-900 dark:text-gray-100 ${density === "ultra-compact" ? "text-[12px]" : ""}`}>{formatCurrency(asset.cost)}</p>
        </td>
        ...
        <td className={`${cellPad}`}>
          <div className={`flex items-center ${density === "ultra-compact" ? "gap-1.5" : "gap-2"}`}>
            <Button
              ...
              variant="outline"
              size="sm"
              className={`transition-all duration-200 group rounded-lg hover:bg-[#f3f4f6] dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 ${density === "ultra-compact" ? "p-1.5 border-0" : "p-2"}`}
      ...
      {expandedId === asset.id && (
        <AssetDetails asset={asset} density={density} consentRequired={consentRequired} formatCurrency={formatCurrency} resolveTypeName={resolveTypeName} />
      )}
    </>
  );
}
