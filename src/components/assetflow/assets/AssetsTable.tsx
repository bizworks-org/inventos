              ...
              className={`
                      ${iconBox} rounded-lg flex items-center justify-center font-semibold
                        ${(() => {
                          ...
                          if (cat.includes("server") || cat.includes("storage"))
            ...
            >
              {(() => {
                const name =
                  ...
                  String((asset as any).type_id ?? (asset as any).type ?? "");
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {(() => {
                ...
                // CIA values
                const cia_c = Number(
                  (asset as any).ciaConfidentiality ??
                    (asset as any).cia_confidentiality ??
                    0
                );
                const cia_i = Number(
                  (asset as any).ciaIntegrity ??
                    (asset as any).cia_integrity ??
                    0
                );
                const cia_a = Number(
                  (asset as any).ciaAvailability ??
                    (asset as any).cia_availability ??
                    0
                );
                ...
                push(
                  "Serial Number",
                  v((asset as any).serialNumber || (asset as any).serial_number)
                );
                push(
                  "Assigned To",
                  v((asset as any).assignedTo || (asset as any).assigned_to)
                );
                push(
                  "Assigned Email",
                  v(
                    (asset as any).assignedEmail ||
                      (asset as any).assigned_email
                  )
                );
                push(
                  "Consent Status",
                  v(
                    (asset as any).consentStatus ||
                      (asset as any).consent_status
                  )
                );
                push(
                  "Department",
                  v((asset as any).department || (asset as any).dept)
                );
                ...
                push(
                  ...
                  v(
                    formatCurrency(
                      Number((asset as any).cost ?? (asset as any).price ?? 0)
                    )
                  )
                );
                ...
                if (specs && Object.keys(specs).length) {
                  ...
                  push(
                    "Specifications",
                    <div className="flex flex-col gap-1">{specNodes}</div>
                  );
                } else {
                  push(
                    "Specifications",
                    v((asset as any).specifications || "—")
                  );
                }
                ...
                push(
                  "Created At",
                  tryDate((asset as any).created_at || (asset as any).createdAt)
                );
                push(
                  "Updated At",
                  tryDate((asset as any).updated_at || (asset as any).updatedAt)
                );
