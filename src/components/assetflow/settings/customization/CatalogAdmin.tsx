...
export default function CatalogAdmin({}: {}) {
  ...
  const gradientForCategory = (name?: string) => {
    ...
    if (n.includes("workstation"))
      return "linear-gradient(to bottom right, #6366f1, #8b5cf6)";
    if (n.includes("server") || n.includes("storage"))
      return "linear-gradient(to bottom right, #06b6d4, #3b82f6)";
    if (n.includes("network"))
      return "linear-gradient(to bottom right, #10b981, #14b8a6)";
    if (n.includes("accessor"))
      return "linear-gradient(to bottom right, #f59e0b, #f97316)";
    if (n.includes("electronic"))
      return "linear-gradient(to bottom right, #ec4899, #f43f5e)";
  ...
  const gradientForType = (name: string) => {
    ...
    if (n.includes("laptop") || n.includes("desktop") || n.includes("notebook"))
      return "linear-gradient(to right, #6366f1, #8b5cf6)"; // indigo → violet
    if (n.includes("server") || n.includes("storage"))
      return "linear-gradient(to right, #06b6d4, #3b82f6)"; // cyan → blue
    if (n.includes("monitor") || n.includes("display") || n.includes("screen"))
      return "linear-gradient(to right, #f59e0b, #f97316)"; // amber → orange
    if (n.includes("printer"))
      return "linear-gradient(to right, #14b8a6, #10b981)"; // teal → emerald
    if (n.includes("phone") || n.includes("mobile") || n.includes("smart"))
      return "linear-gradient(to right, #ec4899, #f43f5e)"; // pink → rose
  ...
  const addType = async () => {
    ...
    try {
      const res = await fetch("/api/admin/catalog", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, categoryId: selected.id }),
      });
      const data = await res.json();
  ...
  const saveRenameCategory = async (categoryId: number, nameRaw: string) => {
    ...
    try {
      const res = await fetch("/api/admin/catalog", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ entity: "category", id: categoryId, name }),
      });
      const data = await res.json();
  ...
  const submitRenameType = async () => {
    ...
    try {
      ...
      if (renamingTypeCategoryId != null)
        body.categoryId = renamingTypeCategoryId;
      const res = await fetch("/api/admin/catalog", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const sortType = async (typeId: number, nextSort: number) => {
    ...
    try {
      const res = await fetch("/api/admin/catalog", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ entity: "type", id: typeId, sort: nextSort }),
      });
      const data = await res.json();
    ...
    try {
      ...
      if (target.entity === "category") {
        ...
        if (typeIds.length) {
          ...
          const dryRuns = await Promise.all(
            typeIds.map((tid) =>
              fetch("/api/admin/catalog", {
                method: "DELETE",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ entity: "type", id: tid, dryRun: true }),
              })
            )
          );
          for (const r of dryRuns) {
            ...
            if (!r.ok)
              throw new Error(dd?.error || "Failed to check type dependencies");
          ...
          setCategories((prev) =>
            prev.map((c) => (c.id === target.id ? { ...c, types: [] } : c))
          );
          try {
      ...
      setCategories((prev) => {
        if (target.entity === "category") {
          ...
          try {
            localStorage.setItem("catalog.categories", JSON.stringify(next));
          } catch (e) {}
          return next;
        } else {
          ...
          try {
            localStorage.setItem("catalog.categories", JSON.stringify(out));
          } catch (e) {}
          return out;
        }
      });
      ...
      toast.success(
        `${target.entity === "category" ? "Category" : "Type"} deleted`
      );
  ...
  return (
    <div>
                ...
                {categories.map((c) => {
                  ...
                  if (isEditing) {
                    return (
                      <div
                        key={c.id}
                        className="w-full flex items-center justify-between px-3 py-2 rounded-lg border bg-[#f8f9ff] border-[#e2e8f0]"
                      >
                        <div className="flex items-center gap-2 min-w-0 flex-1">
                          <span
                            className="inline-flex h-7 w-7 items-center justify-center rounded-md text-white"
                            style={{
                              backgroundImage: gradientForCategory(c.name),
                            }}
                          >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            console.debug("[Catalog] delete category click", {
                              id: c.id,
                              name: c.name,
                            });
                            setConfirmPayload({
                        ...
                        {!isEditing ? (
                          ...
                          >
                            ...
                            <span className="flex items-center gap-2">
                              <button
                                onClick={() =>
                                  startRenameType(t.id, t.name, selected.id)
                                }
                              ...
                              <button
                                onClick={() =>
                                  sortType(t.id, Math.max(0, (t.sort || 0) - 5))
                                }
                          ...
                          <div className="w-full flex items-center gap-2 p-2 rounded-lg border bg-[#f8f9ff]">
                            <input
                              value={renamingTypeName}
                              onChange={(e) =>
                                setRenamingTypeName(e.target.value)
                              }
                              className="flex-1 px-3 py-2 rounded-lg bg-white border border-[#e2e8f0]"
                            />
                            <select
                              value={renamingTypeCategoryId ?? selected.id}
                              onChange={(e) =>
      ...
      {/* debug: render confirm dialog state */}
      {confirmOpen && (
        <Portal>
                  ...
                  pendingTypes.length > 0 && (
                    <div className="mb-3">
                      <p className="text-sm text-[#64748b]">
                        ...
                        type(s). These types will be deleted along with the
                        category if no assets reference them.
                      </p>
                    </div>
                  )}

                {confirmPayload &&
                typeof confirmPayload.count === "number" &&
                confirmPayload.count > 0 ? (
                  <div className="p-4 bg-[#fff7ed] rounded-md border border-[#ffedd5] mb-2">
                    <p className="text-sm text-[#92400e]">
                      There are{" "}
                      <span className="font-semibold">
                        {confirmPayload.count}
                      </span>{" "}
                    ...
                    {confirmPayload.types &&
                      confirmPayload.types.length > 0 && (
                        <p className="text-xs text-[#92400e] mt-2">
                          Affected types: {confirmPayload.types.join(", ")}
                        </p>
                      )}
                    <p className="text-xs text-[#92400e] mt-2">
                ...
                <div className="mt-6 flex items-center justify-end gap-3">
                  ...
                  confirmPayload.count > 0 ? (
                    ...
                    <div className="text-sm text-[#92400e]">
                      This item cannot be deleted because there are assets
                      assigned. Reassign or remove assets first.
                    </div>
                  ) : (
                    <button
