...
import { dbGetRolePermissions, dbSetRolePermissions, dbFindUserById } from '@/lib/auth/db-users';
...
async function requireAdmin() {
  ...
  try {
    ...
    if (user && Array.isArray(user.roles) && user.roles.includes('admin')) return me;
  } catch {}
  return null;
}
