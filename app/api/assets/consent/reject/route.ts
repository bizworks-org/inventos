...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (row.action !== 'reject') return NextResponse.json(renderHtml('Incorrect link.', false), { status: 400, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
    // Update asset
    await query(`UPDATE assets SET consent_status='rejected' WHERE id=:id`, { id: row.asset_id });
    await query(`UPDATE asset_consent_tokens SET used_at = NOW() WHERE token = :t`, { t: token });
    return new NextResponse(renderHtml('You have rejected the assignment.', true), { headers: { 'Content-Type': 'text/html; charset=utf-8' } });
  } catch (e: any) {
