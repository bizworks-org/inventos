...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (recipients.length) {
      await notify({ type: 'license.created', title: `License created: ${body.name}`, body: `${me?.email || 'system'} created license ${body.name}`, recipients, entity: { type: 'license', id: String(body.id) }, metadata: { id: body.id, name: body.name } });
    }
  } catch {}
