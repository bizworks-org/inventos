...
export async function GET(_req: NextRequest, ctx: any) {
  ...
  const rows = await query(
    "SELECT id, `type`, name, created_at FROM vendor_documents WHERE vendor_id = :vendor_id ORDER BY created_at DESC",
    { vendor_id: id }
  );
...
export async function POST(req: NextRequest, ctx: any) {
  ...
  const res = await query(
    "INSERT INTO vendor_documents (vendor_id, `type`, name, `blob`) VALUES (:vendor_id, :type, :name, :blob)",
    { vendor_id: id, type, name: origName, blob: buffer }
  );
  return NextResponse.json({
    ok: true,
